...
Seek requests debounce for 110 ms, cancel previous work and produce one composited frame. Revision/request guards prevent stale frames publishing after edits, switches or newer seeks. Playback prepares the entire remaining timeline from the current playhead into one file and uses one WPF MediaElement for synchronized A/V. Native clock positions map to global timeline seconds. There are no periodic 20-second player swaps or render pauses; the tradeoff is more initial preparation and temporary disk space for long projects. This is rendered playback, not realtime per-track decoding.
...
The 68 render checks include 26 audio-continuity checks. These compare every decoded PCM sample before and after multiple cuts for 44.1/48 kHz WAV and MP3 sources, PCM/MP3/AAC preview outputs, exact/half/sub-sample cuts, independent overlapping fades, range alignment and MUTE. Equality is against an unsplit render using the same codec/settings, not against lossless original samples for lossy MP3/AAC outputs.

...
Visual seek requests debounce for 110 ms, cancel previous work and produce one composited frame. Audio-only seeks need no frame render. Revision/request guards prevent stale frames publishing after edits, switches or newer seeks. Playback prepares the entire timeline from zero into one file and uses one WPF MediaElement for synchronized A/V. Audio-only projects produce stereo 48 kHz PCM16 WAV instead of encoding black video. Backward seek, Stop, EOF replay and repeated Prepare reuse the loaded player; navigation/Stop during preparation clear autoplay without canceling the render. Explicit Cancel or changed media content still cancels obsolete work.
...
`AudioPreviewCache` retains whole-source 48 kHz stereo float64 decoded PCM on disk for preview renders, preserving the original double-precision resampling sequence across subsequent trims and effects. Leases protect in-use files; path, source metadata and decoder identity detect stale entries. Completed files are published atomically, cancellation removes partial output and bounded LRU cleanup limits storage. Preview mixes still run through the same sample-accurate audio graph. Export continues decoding original sources independently. There are no periodic player swaps or render pauses; long projects still require initial preparation, and effect edits still require mixing. This remains rendered playback rather than realtime per-track decoding.
...
`scripts/verify.ps1` builds Release, runs standalone core checks, offscreen WPF workflows and real FFmpeg render checks. Coverage includes native import, placement, inspector/custom Fade edits, batch/link operations, scrolled marquee coordinates, all-member move ghosts, mixed-kind group movement, snapping controls, fractional-rate navigation, quarter-width zoom, editable 30 px rows, MUTE, routed Space and pending autoplay, lock/overlap rejection, Undo/Redo, recovery isolation/races, relink, real waveforms, source hashes, save/reopen, global preview seeking, continuous playback past the former 20-second boundary, EOF frames, output settings and small-window layout.
